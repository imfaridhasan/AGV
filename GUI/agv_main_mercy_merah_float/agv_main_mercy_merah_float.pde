...
//data-data
int switchbuttonvar;
float baterai1raw=12.5,baterai2raw=7.45;
float baterai=map(baterai1raw,0,12.96,0,100);
float baterai1=map(baterai2raw,0,12.6,0,100);
int jmlsatelit=10;
float[] datamasuk;
float  jarak, jarakpertama, roll, pitch;
float tklatnow, tklonnow,lat, lng;
...
public void setup() {
  ...
  cp5.addButton("switchbutton")
   ...
   cp5.addButton("sendbutton")
    .setPosition(width-115, 110)
      .setSize(80, 20)
        .setColorBackground(0) 
          .setColorForeground(color( 24, 179, 113 ) )
            .setCaptionLabel("Send Command") 
              .setColorActive( color( 24, 179, 113 ) )
                  ;
   cp5.addButton("startmission")
    .setPosition(width-115, 140)
      .setSize(80, 20)
        .setColorBackground(0) 
          .setColorForeground(color( 24, 179, 113 ) )
            .setCaptionLabel("Start Mission") 
              .setColorActive( color( 24, 179, 113 ) )
                  ;
...
public void draw() {
   baterai=map(baterai1raw,0,12.6,0,100);
   baterai1=map(baterai2raw,0,12.6,0,100);
...
void mouseClicked() {
  ...
  else if((mouseX > 1140)&&(mouseY < 200)){
...
void sendbutton(boolean theFlag) {
  nilaiperintah=0;
  if(i>0){
  serial.write(kirim);
  }
...
void startmission(boolean theFlag) {
  //println(nilaiperintah);
  if(nilaiperintah>=1){
    nilaiperintah=0;
  }
  else{
    nilaiperintah=nilaiperintah+1;
  }
  if(i>0){
  serial.write(kirim);
  }
